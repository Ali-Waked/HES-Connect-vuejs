...
const categoryColors = {
  ...
  Surgery: 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400',
  Obstetrics: 'bg-pink-100 text-pink-700 dark:bg-pink-900/30 dark:text-pink-400',
  'Health Policy': 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400'
};
</script>
  ...
  <LandingSection class="py-16 lg:py-20 bg-landing-cream dark:bg-slate-900">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex items-center justify-between mb-8">
        <h2 class="text-2xl sm:text-3xl font-bold text-slate-900 dark:text-slate-100">Latest Medical Articles</h2>
      ...
      <div v-else class="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
        <article
          ...
          :key="article.id"
          class="bg-white dark:bg-slate-800 rounded-xl border border-landing-border dark:border-slate-700 p-6 shadow-sm hover:shadow-md hover:-translate-y-0.5 hover:border-brand-primary/20 transition-all duration-200"
        >
          ...
          <h3 class="text-base font-bold text-slate-900 dark:text-slate-100 leading-snug line-clamp-2 mb-3">{{ article.title }}</h3>
          <p class="text-sm text-slate-500 dark:text-slate-400 mb-1">{{ article.author_name }}</p>
          <p class="text-xs text-slate-400 dark:text-slate-500 mb-4">{{ article.published_at }}</p>
          ...
          <div class="flex flex-wrap gap-1.5 mb-4">
            <span
              ...
              :key="tag.name"
              class="px-2 py-0.5 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-400 text-xs font-medium rounded-md"
            >{{ tag.name }}</span>
          </div>
