    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex items-end justify-between mb-12 animate-fade-in-up">
        <div>
          ...
          <h2 class="text-3xl sm:text-4xl font-bold text-slate-900">Healthcare Articles</h2>
          <p class="text-slate-500 mt-2">Stay informed with the latest healthcare news and insights.</p>
        </div>
        <button
      ...
      <div v-if="loading" class="grid lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 rounded-xl border border-slate-200 overflow-hidden animate-pulse">
          <div class="h-72 bg-slate-100"></div>
        </div>
        <div class="space-y-6">
          <div v-for="n in 2" :key="n" class="rounded-xl border border-slate-200 overflow-hidden animate-pulse">
      ...
      <div v-else class="grid lg:grid-cols-3 gap-6 lg:gap-8">
        ...
        >
          <div class="grid sm:grid-cols-5 h-full">
            ...
            <div class="sm:col-span-2 p-6 lg:p-8 flex flex-col justify-center">
              ...
              <h3 class="text-lg lg:text-xl font-bold text-slate-900 group-hover:text-brand-primary transition-colors line-clamp-2 mb-2">{{ localField(articles[0], 'title') }}</h3>
              <p class="text-sm text-slate-500 line-clamp-3 leading-relaxed">{{ localField(articles[0], 'excerpt') || localField(articles[0], 'summary') || '' }}</p>
        ...
        <div class="space-y-6 lg:space-y-8">
          ...
          >
            ...
            <div class="p-5">
              <h3 class="text-base font-bold text-slate-900 group-hover:text-brand-primary transition-colors line-clamp-2 mb-1">{{ localField(article, 'title') }}</h3>
              <p class="text-sm text-slate-500 line-clamp-2 leading-relaxed">{{ localField(article, 'excerpt') || localField(article, 'summary') || '' }}</p>
