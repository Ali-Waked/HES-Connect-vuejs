...
const typeBadge = {
  ...
  medical_point: 'bg-orange-100 text-orange-700 border-orange-200'
};
  ...
  <LandingSection class="py-16 lg:py-20 bg-landing-cream">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex items-center justify-between mb-8">
        <h2 class="text-2xl sm:text-3xl font-bold text-slate-900">Top Rated Facilities</h2>
        <button class="text-sm font-semibold text-brand-primary hover:text-brand-primary-hover transition cursor-pointer" @click="router.push('/facilities')">View All →</button>
      </div>
      ...
      <div v-else class="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
        <div
          ...
          :key="fac.id"
          class="bg-white rounded-xl border border-landing-border overflow-hidden shadow-sm hover:shadow-md hover:-translate-y-0.5 hover:border-brand-primary/20 transition-all duration-200"
        >
          <div :class="`h-36 bg-gradient-to-br ${gradients[idx % gradients.length]} flex items-center justify-center`">
          ...
          <div class="p-5 space-y-3">
            <span :class="`inline-flex px-2.5 py-0.5 rounded-full text-xs font-semibold border capitalize ${typeBadge[fac.facility_type] || 'bg-slate-100 text-slate-600'}`">
              {{ fac.facility_type?.replace('_', ' ') }}
            </span>
            <h3 class="text-base font-bold text-slate-900 leading-snug">{{ fac.name }}</h3>
            <p class="text-sm text-slate-500">{{ fac.organization_name }}</p>
            <div class="flex items-center gap-2 text-sm">
              ...
              <span class="font-semibold text-slate-700">{{ fac.rating }}</span>
              <span class="text-slate-400">({{ fac.review_count }} reviews)</span>
            </div>
            ...
            <button
              class="w-full py-2.5 text-sm font-semibold text-brand-primary border border-brand-primary rounded-lg hover:bg-brand-primary/5 transition cursor-pointer"
