...
<template>
  <div class="space-y-6">
    ...
    <div class="flex justify-between items-start">
      <div class="space-y-1">
        <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Organizations</h1>
        <p class="text-sm text-slate-500">{{ filteredCount }} registered {{ filteredCount === 1 ? 'organization' : 'organizations' }}</p>
      </div>
      <button 
    ...
    <div class="bg-white rounded-xl border border-slate-200 shadow-xs overflow-hidden">
      ...
      <div v-if="filteredOrgs.length === 0" class="flex flex-col items-center justify-center py-12 px-6 text-center gap-4">
        ...
        <p class="text-sm font-semibold text-slate-600">No organizations found matching your criteria.</p>
        ...
        <table class="w-full border-collapse text-left">
          <thead>
            <tr class="bg-slate-50 border-b border-slate-200">
              <th class="px-6 py-4 text-[11px] font-bold text-slate-500 uppercase tracking-wider w-[45%]">Name</th>
              <th class="px-6 py-4 text-[11px] font-bold text-slate-500 uppercase tracking-wider w-[15%]">Type</th>
              <th class="px-6 py-4 text-[11px] font-bold text-slate-500 uppercase tracking-wider w-[15%]">Created</th>
              <th class="px-6 py-4 text-[11px] font-bold text-slate-500 uppercase tracking-wider w-[15%]">Updated</th>
              <th class="px-6 py-4 text-[11px] font-bold text-slate-500 uppercase tracking-wider w-[10%] text-right">Actions</th>
            </tr>
          </thead>
            ...
            >
              <td class="px-6 py-4.5">
                <div class="flex flex-col gap-0.5">
                  <span class="text-sm font-semibold text-slate-900 leading-normal">{{ org.name }}</span>
                  <span class="text-xs text-slate-500 leading-normal">{{ org.description }}</span>
                </div>
              </td>
              ...
              <td class="px-6 py-4.5 whitespace-nowrap text-sm text-slate-600">
                {{ org.created }}
              </td>
              <td class="px-6 py-4.5 whitespace-nowrap text-sm text-slate-600">
                {{ org.updated }}
