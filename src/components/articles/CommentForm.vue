  ...
  <div class="card-base p-5">
    <template v-if="isAuthenticated">
      ...
      <div class="flex items-center justify-between mt-3">
        <p class="text-xs text-slate-400 dark:text-slate-500">{{ modelValue.length }} / 500</p>
        <button
          class="btn-primary"
          :disabled="submitting"
          @click="emit('submit')"
        >
          ...
          <span v-else>{{ t('comment.postComment') }}</span>
        </button>
      </div>
    </template>
    <template v-else>
      <div class="text-center py-6">
        ...
        <p class="text-sm text-slate-500 dark:text-slate-400 mb-3">{{ t('comment.loginToComment') }}</p>
        <button
          class="btn-primary"
          @click="goToLogin"
        >
