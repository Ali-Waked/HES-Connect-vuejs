...
<template>
  <section v-if="related.length">
    <h3 class="text-lg font-bold text-slate-900 dark:text-slate-100 mb-4">Other Stories That Need Support</h3>
    <div class="flex gap-4 overflow-x-auto pb-2 snap-x">
      <div v-for="item in related" :key="item.id"
        class="min-w-[260px] flex-shrink-0 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-sm transition hover:shadow-md snap-start">
        <div class="h-36 overflow-hidden rounded-t-xl bg-slate-100 dark:bg-slate-700">
        ...
        <div class="p-4 space-y-2">
          <p class="text-sm font-bold text-slate-900 dark:text-slate-100">{{ item.patientFirstName }}, {{ item.patientAge }}</p>
          <p class="text-xs text-slate-500 dark:text-slate-400 line-clamp-2">{{ item.content.split('\n')[0].slice(0, 100) }}...</p>
          <div v-if="item.isFundraising" class="space-y-1">
            ...
            <p class="text-xs text-slate-500 dark:text-slate-400">{{ fmt(item.collectedAmount) }} of {{ fmt(item.targetAmount) }}</p>
          </div>
