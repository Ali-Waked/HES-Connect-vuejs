...
<template>
  <div class="space-y-3">
    <div class="relative overflow-hidden rounded-xl bg-slate-100 aspect-video">
      <img v-if="currentImage" :src="currentImage" :alt="title"
        class="h-full w-full object-cover transition-opacity duration-300"
        :key="activeIndex" />
      <div v-else
        class="flex h-full w-full items-center justify-center bg-gradient-to-br from-teal-50 to-slate-100">
    ...
    <div v-if="images?.length > 1" class="flex gap-2">
      <button v-for="(img, i) in images.slice(0, 4)" :key="i"
        ...
        :class="i === activeIndex ? 'border-brand-primary ring-2 ring-brand-primary/20' : 'border-slate-200'"
        @click="select(i)">
