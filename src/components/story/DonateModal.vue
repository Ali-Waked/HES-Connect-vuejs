    ...
    <div class="w-full max-w-md rounded-xl border border-slate-200 bg-white shadow-2xl">
      <div class="flex items-center justify-between border-b border-slate-100 px-5 py-4">
        <div class="flex items-center gap-2">
          ...
          <h2 class="text-base font-bold text-slate-900">Support {{ story?.patientFirstName }}'s Story</h2>
        </div>
      ...
      <div class="space-y-5 p-5">
        <div>
          <label class="mb-2 block text-sm font-semibold text-slate-700">Choose an amount</label>
          <div class="flex gap-2">
            <button v-for="amount in quickAmounts" :key="amount"
              ...
              :class="selectedQuick === amount ? 'border-brand-primary bg-brand-primary/5 text-brand-primary' : 'border-slate-200 text-slate-600 hover:border-slate-300'"
              @click="pickQuick(amount)">${{ amount }}</button>
          </div>
        </div>
        <div>
          <label class="mb-1 block text-sm font-semibold text-slate-700">Or enter custom amount</label>
          <div class="relative">
            ...
            <input type="number" min="1" v-model.number="customAmount" placeholder="Enter amount"
              class="w-full rounded-lg border border-slate-200 py-2.5 pl-7 pr-3 text-sm" @input="onCustomInput" />
          </div>
        </div>
        <div>
        ...
        <div>
          <label class="mb-1 block text-sm font-semibold text-slate-700">Your Name <span class="text-slate-400 font-normal">(optional)</span></label>
          <input v-model="donorName" placeholder="Anonymous" class="w-full rounded-lg border border-slate-200 px-3 py-2.5 text-sm" />
        </div>
        <p v-if="error" class="text-xs text-red-500">{{ error }}</p>
        <div class="flex gap-3 pt-1">
          <button class="flex-1 rounded-lg border border-slate-200 px-4 py-2.5 text-sm font-semibold text-slate-700 transition hover:bg-slate-50" @click="$emit('close')">Cancel</button>
