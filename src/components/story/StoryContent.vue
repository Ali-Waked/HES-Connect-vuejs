  ...
  <div class="relative">
    <div class="transition-all duration-300" :class="{ 'max-h-none': expanded, 'max-h-[600px] overflow-hidden': isLong && !expanded }">
      <div v-for="(paragraph, i) in displayContent.split('\n').filter(p => p.trim())" :key="i"
        class="mb-4 text-base leading-relaxed text-slate-700 dark:text-slate-300 last:mb-0">
        {{ paragraph.trim() }}
      </div>
    </div>
    <div v-if="isLong && !expanded" class="absolute bottom-0 left-0 right-0 h-24 bg-gradient-to-t from-white dark:from-slate-900 to-transparent"></div>
