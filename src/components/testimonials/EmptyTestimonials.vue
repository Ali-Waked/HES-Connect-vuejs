...
<template>
  <div class="bg-white dark:bg-slate-800 rounded-2xl border border-dashed border-slate-300 dark:border-slate-600 py-16 px-6 text-center">
    <div class="w-16 h-16 mx-auto mb-4 bg-slate-50 dark:bg-slate-700 rounded-full flex items-center justify-center">
    ...
    <h3 class="text-lg font-bold text-slate-700 dark:text-slate-300 mb-1">{{ t('testimonials.emptyTitle') }}</h3>
    <p class="text-sm text-slate-500 dark:text-slate-400 max-w-md mx-auto">{{ t('testimonials.emptyDesc') }}</p>
  </div>
</template>
