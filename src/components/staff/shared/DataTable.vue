  ...
  <div class="overflow-hidden rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-sm">
    <div class="overflow-x-auto">
      <table class="w-full text-left">
        <thead class="border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900">
          <tr>
        ...
        <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
          ...
          <template v-else-if="items.length">
            <tr v-for="item in items" :key="item.id" class="transition hover:bg-slate-50/70 dark:hover:bg-slate-700/70">
          ...
          <tr v-else>
            <td :colspan="columns.length" class="px-5 py-12 text-center">
              <span class="material-symbols-outlined text-4xl text-slate-300 dark:text-slate-500">inventory_2</span>
              <h3 class="mt-2 text-sm font-bold text-slate-900 dark:text-slate-100">{{ emptyTitle }}</h3>
