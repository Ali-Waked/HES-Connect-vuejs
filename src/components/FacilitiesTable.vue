...
const filteredFacilities = computed(() => {
  return store.facilities.filter(fac => {
    ...
    const matchesSearch = !query || 
      localField(fac, 'name').toLowerCase().includes(query) || 
...
<template>
  <div class="space-y-6">
    ...
    <div class="flex justify-between items-start">
      <div class="space-y-1">
        <h1 class="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Facilities</h1>
          ...
          <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
            ...
            >
              <td class="px-6 py-4.5">
                <span class="text-sm font-semibold text-slate-900 dark:text-white leading-normal">{{ localField(facility, 'name') }}</span>
              </td>
              ...
              <td class="px-6 py-4.5 whitespace-nowrap text-sm font-medium text-slate-600 dark:text-slate-300">
    ...
    <ConfirmModal 
      ...
      title="Delete Facility"
      :message="`Are you sure you want to delete <strong>${localField(selectedFacility, 'name')}</strong>? This action cannot be undone.`"
      confirm-text="Delete"
