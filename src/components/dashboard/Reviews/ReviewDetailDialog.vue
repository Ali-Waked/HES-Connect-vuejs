    ...
    <div v-if="review" class="space-y-6">
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 bg-slate-50 rounded-xl border border-slate-100">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h4 class="text-sm font-bold text-slate-900">{{ review.user }}</h4>
            <div class="flex items-center gap-2">
              <RatingStars :rating="review.rating" size="xs" />
              <span class="text-[10px] text-slate-400 font-medium">{{ formatDate(review.created_at) }}</span>
            </div>
          </div>
        </div>
      ...
      <div class="space-y-2">
        <label class="text-xs font-bold text-slate-500 uppercase tracking-wider">Comment</label>
        <div class="p-4 bg-white border border-slate-200 rounded-xl text-sm text-slate-700 leading-relaxed italic">
          "{{ review.comment }}"
        </div>
      </div>
      ...
      <div class="flex justify-end gap-3 pt-4 border-t border-slate-100">
        <button 
          ...
          @click="hide"
          class="inline-flex items-center justify-center gap-2 py-2 px-4 bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 text-sm font-bold rounded-lg transition"
        >
