  ...
  >
    <div v-if="invoice" class="space-y-6">
      <div class="flex justify-between items-start border-b border-slate-100 pb-4">
        <div>
          <h4 class="text-xl font-bold text-slate-900">{{ invoice.id }}</h4>
          <p class="text-xs text-slate-500 font-medium uppercase tracking-wider mt-1">Patient: {{ localField(invoice, 'patient_name') }}</p>
        </div>
        <div class="text-right">
          <BaseBadge :variant="invoice.status === 'paid' ? 'success' : 'danger'">
            {{ invoice.status.toUpperCase() }}
          </BaseBadge>
          <p class="text-xs text-slate-400 mt-2">{{ formatDate(invoice.created_at) }}</p>
        </div>
      </div>

      <div class="grid grid-cols-2 gap-4">
        <div class="p-4 bg-slate-50 rounded-xl">
          <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Appointment ID</p>
          <p class="text-sm font-bold text-slate-900">{{ invoice.appointment_id }}</p>
        </div>
      ...
      <div class="space-y-3">
        <label class="text-xs font-bold text-slate-500 uppercase tracking-wider">Line Items</label>
        <div class="border border-slate-200 rounded-xl overflow-hidden">
          <table class="w-full text-sm">
            <thead class="bg-slate-50 border-b border-slate-200 text-left">
              <tr>
                <th class="px-4 py-2 font-bold text-slate-600">Description</th>
                <th class="px-4 py-2 font-bold text-slate-600 text-right">Price</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-slate-100">
              <tr v-for="(item, idx) in invoice.items" :key="idx">
                <td class="px-4 py-2.5 text-slate-700">{{ localField(item, 'description') }}</td>
                <td class="px-4 py-2.5 text-slate-900 font-medium text-right">{{ formatCurrency(item.price) }}</td>
              </tr>
            </tbody>
            <tfoot class="bg-slate-50 border-t border-slate-200 font-bold">
              <tr>
                <td class="px-4 py-2 text-slate-900">Total</td>
      ...
      <div class="flex justify-end gap-3 pt-4 border-t border-slate-100">
        ...
        <button 
          class="inline-flex items-center justify-center gap-2 py-2 px-6 bg-slate-100 text-slate-700 hover:bg-slate-200 text-sm font-bold rounded-lg transition"
          @click="window.print()"
        >
