...
<template>
  <div class="space-y-6 animate-fade-in">
    ...
    <div class="flex justify-between items-start">
      <div class="space-y-1">
        <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Departments</h1>
        <p class="text-sm text-slate-500">{{ store.deptCount }} {{ store.deptCount === 1 ? 'department' : 'departments' }} across all facilities</p>
      </div>
      <button
    ...
    <!-- ── Search bar ─────────────────────────────────────────────── -->
    <div class="relative">
      <svg class="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 pointer-events-none" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    <div class="bg-white rounded-xl border border-slate-200 shadow-xs overflow-hidden">
      ...
      <div v-if="filtered.length === 0" class="flex flex-col items-center justify-center py-20 gap-3 text-center">
        ...
        <p class="text-sm font-semibold text-slate-600">No departments match your search.</p>
        <button class="py-2 px-4 rounded-lg bg-slate-100 hover:bg-slate-200 border border-slate-200 text-xs font-semibold text-slate-600 transition cursor-pointer" @click="searchQuery = ''">Clear Search</button>
      </div>
        ...
        <table class="w-full border-collapse text-left">
          <thead>
            <tr class="border-b border-slate-200">
              <th class="px-6 py-4 text-[11px] font-bold text-slate-500 uppercase tracking-wider">Name</th>
              <th class="px-6 py-4 text-[11px] font-bold text-slate-500 uppercase tracking-wider">Facility</th>
              <th class="px-6 py-4 text-[11px] font-bold text-slate-500 uppercase tracking-wider">Department Head</th>
            ...
            >
              ...
              <td class="px-6 py-4">
                <div class="flex items-center gap-3">
                  ...
                  <div class="w-8 h-8 rounded-lg bg-slate-100 flex items-center justify-center flex-shrink-0">
                    <svg class="w-4 h-4 text-slate-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
              ...
              <td class="px-6 py-4">
                <div class="flex items-center gap-2.5">
                  ...
                  <span class="text-sm font-medium text-slate-700">{{ dept.head }}</span>
                </div>
              </td>

              <!-- Created date -->
              <td class="px-6 py-4 text-sm text-slate-500 whitespace-nowrap">{{ dept.created }}</td>
