...
<template>
  ...
  >
    <div class="bg-white dark:bg-slate-800 rounded-xl w-full max-w-[480px] shadow-2xl border border-slate-200 dark:border-slate-700 animate-slide-up overflow-hidden">

      <!-- Header -->
      <div class="flex justify-between items-center p-5 px-6 border-b border-slate-100 dark:border-slate-700">
        <h3 class="text-lg font-bold text-slate-900 dark:text-slate-100">
          {{ mode === 'edit' ? 'Edit Department' : 'Add Department' }}
        </h3>
        <button class="text-slate-400 dark:text-slate-500 hover:text-slate-600 hover:bg-slate-100 dark:hover:bg-slate-700 p-1 rounded-lg cursor-pointer transition" @click="$emit('close')">
      ...
      <form @submit.prevent="submit">
        <div class="p-6 flex flex-col gap-4">
          ...
          <div class="flex flex-col gap-1.5">
            <label class="text-xs font-semibold text-slate-600 dark:text-slate-400" for="dName">Department Name *</label>
            <input
              id="dName"
              v-model="name"
              type="text"
              ...
              class="w-full p-2.5 text-sm border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none transition"
            />
          </div>

          <!-- Facility -->
          <div class="flex flex-col gap-1.5">
            <label class="text-xs font-semibold text-slate-600 dark:text-slate-400" for="dFacility">Facility *</label>
            <select
              id="dFacility"
              v-model="facility"
              required
              class="w-full p-2.5 text-sm border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none cursor-pointer transition bg-white dark:bg-slate-800"
            >
          ...
          <!-- Department Head -->
          <div class="flex flex-col gap-1.5">
            <label class="text-xs font-semibold text-slate-600 dark:text-slate-400" for="dHead">Department Head *</label>
            <input
              id="dHead"
              v-model="head"
              type="text"
