  ...
  <div v-if="show" class="fixed inset-0 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center z-[100] p-4" @click.self="$emit('close')">
    <div class="bg-white dark:bg-slate-800 rounded-xl w-full max-w-[520px] shadow-2xl border border-slate-200 dark:border-slate-700 animate-slide-up overflow-hidden">
      ...
      <div class="p-5 px-6 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center">
        ...
        <button class="text-slate-400 dark:text-slate-500 hover:text-slate-600 p-1 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg cursor-pointer transition" @click="$emit('close')">
      ...
      <div v-if="activeMode === 'view'" class="p-6 space-y-5">
        <div>
          <h2 class="text-xl font-bold text-slate-900 dark:text-slate-100 mb-2">{{ story?.title }}</h2>
        ...
        <div class="pt-4 border-t border-slate-100 dark:border-slate-700 flex justify-end gap-3">
          <button type="button" class="py-2 px-4.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-sm font-semibold text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700 transition cursor-pointer" @click="$emit('close')">Close</button>
      ...
      <form v-else @submit.prevent="submitForm">
        <div class="p-6 flex flex-col gap-4">
          ...
          <div class="flex flex-col gap-1.5">
            <label class="text-xs font-semibold text-slate-600 dark:text-slate-400" for="sTitle">Story Title *</label>
            <input id="sTitle" v-model="title" type="text" required placeholder="e.g. Road to Recovery After Surgery" class="w-full p-2.5 text-sm border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none transition"/>
          </div>
          <!-- Patient -->
          <div class="flex flex-col gap-1.5">
            <label class="text-xs font-semibold text-slate-600 dark:text-slate-400" for="sPatient">Patient Name *</label>
            <input id="sPatient" v-model="patient" type="text" required placeholder="e.g. Khalid Abu Amr" class="w-full p-2.5 text-sm border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none transition"/>
          </div>
          <!-- Status -->
          <div class="flex flex-col gap-1.5">
            <label class="text-xs font-semibold text-slate-600 dark:text-slate-400" for="sStatus">Status *</label>
            <select id="sStatus" v-model="status" required class="w-full p-2.5 text-sm border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none cursor-pointer transition">
          ...
          <div v-if="fundraising" class="flex flex-col gap-1.5">
            ...
            <input id="sGoal" v-model.number="goal" type="number" min="0" placeholder="e.g. 5000" class="w-full p-2.5 text-sm border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none transition"/>
          </div>
        </div>
        <div class="p-4 px-6 bg-slate-50 dark:bg-slate-900 border-t border-slate-100 dark:border-slate-700 flex justify-end gap-3">
          <button type="button" class="py-2 px-4.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-sm font-semibold text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700 transition cursor-pointer" @click="$emit('close')">Cancel</button>
