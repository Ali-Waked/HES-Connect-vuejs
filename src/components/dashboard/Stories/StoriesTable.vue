...
<template>
  <div class="space-y-6 animate-fade-in">
    ...
    <div class="flex justify-between items-start">
      <div class="space-y-1">
        <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Patient Stories</h1>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
      ...
      <div class="bg-white rounded-xl border border-slate-200 px-5 py-3.5 flex items-center gap-3 shadow-xs">
        <span class="text-lg font-bold text-slate-900">{{ store.storyCount }}</span>
      ...
      <!-- Fundraising -->
      <div class="bg-white rounded-xl border border-rose-200 px-5 py-3.5 flex items-center gap-3 shadow-xs">
    ...
    <div class="flex flex-col sm:flex-row gap-3">
      <div class="relative flex-grow">
        ...
        <input type="text" v-model="searchQuery" class="w-full pl-10 pr-4 py-2.5 text-sm border border-slate-200 rounded-lg bg-white placeholder-slate-400 focus:outline-none focus:border-brand-primary focus:ring-3 focus:ring-brand-primary/10 transition" placeholder="Search stories..."/>
      </div>
    ...
    <div class="bg-white rounded-xl border border-slate-200 shadow-xs overflow-hidden">
      ...
      <div v-if="filtered.length === 0" class="flex flex-col items-center justify-center py-16 gap-3 text-center">
        ...
        <p class="text-sm font-semibold text-slate-600">No stories match your criteria.</p>
        ...
        <table class="w-full border-collapse text-left">
          <thead>
            <tr class="bg-slate-50 border-b border-slate-200">
              <th class="px-6 py-4 text-[11px] font-bold text-slate-500 uppercase tracking-wider">Story</th>
              ...
              <th class="px-6 py-4 text-[11px] font-bold text-slate-500 uppercase tracking-wider">Fundraising</th>
              <th class="px-6 py-4 text-[11px] font-bold text-slate-500 uppercase tracking-wider">Created</th>
            ...
            <tr v-for="story in filtered" :key="story.id" class="hover:bg-slate-50/50 transition-colors">
              ...
              <td class="px-6 py-4">
                <div class="flex items-center gap-3 min-w-[220px]">
                  ...
                  <div class="min-w-0">
                    <button
                      class="text-sm font-semibold text-slate-900 leading-snug truncate max-w-[200px] hover:text-brand-primary transition cursor-pointer text-left"
                      :title="story.title"
                      @click="openView(story)"
                    >{{ story.title }}</button>
                    <p class="text-xs text-slate-400 mt-0.5">{{ story.images }} {{ story.images === 1 ? 'image' : 'images' }}</p>
                  </div>
                </div>
              </td>
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex items-center gap-2.5">
                  ...
                  <span class="text-sm font-medium text-slate-700">{{ story.patient }}</span>
                </div>
              </td>
              ...
              <td class="px-6 py-4">
                <div v-if="story.fundraising" class="min-w-[120px]">
                  ...
                  <p class="text-xs font-semibold text-slate-700">{{ formatMoney(story.raised) }}</p>
                </div>
                <span v-else class="text-slate-300 text-sm">—</span>
              </td>

              <!-- Created -->
              <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500">{{ story.created }}</td>

              <!-- Actions -->
              <td class="px-6 py-4 whitespace-nowrap text-right">
                <div class="flex justify-end gap-1.5">
                  <button class="text-slate-400 hover:text-slate-700 hover:bg-slate-100 p-1.5 rounded-lg transition cursor-pointer" title="View Story" @click="openView(story)">
                    <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                  </button>
                  <button class="text-slate-400 hover:text-slate-700 hover:bg-slate-100 p-1.5 rounded-lg transition cursor-pointer" title="Edit Story" @click="openEdit(story)">
                  ...
                  <button class="text-slate-400 hover:text-red-600 hover:bg-red-50 p-1.5 rounded-lg transition cursor-pointer" title="Delete Story" @click="confirmDel(story)">
