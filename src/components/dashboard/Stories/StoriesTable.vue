...
// ─── Story cover colour palette (used for placeholder thumbnails) ───────────
const coverColors = [
...
<template>
  <div class="space-y-6 animate-fade-in">
    ...
    <div class="flex justify-between items-start">
      <div class="space-y-1">
        <h1 class="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Patient Stories</h1>
        <p class="text-sm text-slate-500">{{ store.storyCount }} {{ store.storyCount === 1 ? 'story' : 'stories' }} submitted</p>
      </div>
      <button
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
      <!-- Total -->
      <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 px-5 py-3.5 flex items-center gap-3 shadow-xs">
        <span class="text-lg font-bold text-slate-900 dark:text-white">{{ store.storyCount }}</span>
        <span class="text-sm text-slate-500 font-medium">Total</span>
      </div>
      <!-- Approved -->
      <div class="bg-white dark:bg-slate-900 rounded-xl border border-emerald-200 dark:border-emerald-800 px-5 py-3.5 flex items-center gap-3 shadow-xs">
        <span class="text-lg font-bold text-emerald-600 dark:text-emerald-400">{{ store.approvedStoryCount }}</span>
        <span class="text-sm text-slate-500 font-medium">Approved</span>
      </div>
      <!-- Pending -->
      <div class="bg-white dark:bg-slate-900 rounded-xl border border-amber-200 dark:border-amber-800 px-5 py-3.5 flex items-center gap-3 shadow-xs">
        <span class="text-lg font-bold text-amber-600 dark:text-amber-400">{{ store.pendingStoryCount }}</span>
        <span class="text-sm text-slate-500 font-medium">Pending</span>
      </div>
      <!-- Fundraising -->
      <div class="bg-white dark:bg-slate-900 rounded-xl border border-rose-200 dark:border-rose-800 px-5 py-3.5 flex items-center gap-3 shadow-xs">
        <span class="text-lg font-bold text-rose-600 dark:text-rose-400">{{ store.fundraisingStoryCount }}</span>
        <span class="text-sm text-slate-500 font-medium">Fundraising</span>
      </div>
    </div>
    ...
    <div class="flex flex-col sm:flex-row gap-3">
      <div class="relative flex-grow">
        ...
        <input type="text" v-model="searchQuery" class="w-full pl-10 pr-4 py-2.5 text-sm border border-slate-200 dark:border-slate-800 rounded-lg bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none transition" placeholder="Search stories..."/>
      </div>
      <select v-model="statusFilter" class="min-w-[140px] p-2.5 text-sm border border-slate-200 dark:border-slate-800 rounded-lg bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-300 focus:outline-none cursor-pointer transition">
    ...
    <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-xs overflow-hidden transition-colors">
      ...
      <div v-if="filtered.length === 0" class="flex flex-col items-center justify-center py-16 gap-3 text-center">
        ...
        <button class="py-2 px-4 rounded-lg bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 text-xs font-semibold text-slate-600 dark:text-slate-300 transition cursor-pointer" @click="searchQuery='';statusFilter='all'">Reset Filters</button>
      </div>
      ...
      <div v-else class="w-full overflow-x-auto">
        <table class="w-full border-collapse text-left">
          ...
          <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
            <tr v-for="story in filtered" :key="story.id" class="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors">
              <!-- Story Title + Cover -->
              <td class="px-6 py-4">
              ...
              <td class="px-6 py-4">
                <div v-if="story.fundraising" class="min-w-[120px]">
                  <div class="flex items-center gap-1 mb-1.5">
                    ...
                    <span class="text-xs font-bold text-rose-600 dark:text-rose-400">Fundraising</span>
                  </div>
                  <!-- Progress Bar -->
                  <div class="w-full bg-slate-100 dark:bg-slate-800 rounded-full h-1.5 mb-1">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-right">
                <div class="flex justify-end gap-1.5">
                  <button class="text-slate-400 dark:text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 p-1.5 rounded-lg transition cursor-pointer" title="View Story" @click="openView(story)">
                    <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                  </button>
                  <button class="text-slate-400 dark:text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 p-1.5 rounded-lg transition cursor-pointer" title="Edit Story" @click="openEdit(story)">
                  ...
                  <button class="text-slate-400 dark:text-slate-500 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 p-1.5 rounded-lg transition cursor-pointer" title="Delete Story" @click="confirmDel(story)">
