...
// ─── Article cover colour palette (used for placeholder thumbnails) ───────────
const coverColors = [
...
<template>
  <div class="space-y-6 animate-fade-in">
    ...
    <div class="flex justify-between items-start">
      <div class="space-y-1">
        <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Articles Management</h1>
        <p class="text-sm text-slate-500">{{ store.articleCount }} {{ store.articleCount === 1 ? 'article' : 'articles' }} in total</p>
      </div>
      <button
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
      <div class="bg-white rounded-xl border border-slate-200 px-5 py-3.5 flex items-center gap-3 shadow-xs">
        <span class="text-lg font-bold text-slate-900">{{ store.articleCount }}</span>
      ...
      <div class="bg-white rounded-xl border border-emerald-200 px-5 py-3.5 flex items-center gap-3 shadow-xs">
        ...
        <span class="text-sm text-slate-500 font-medium">Published</span>
      </div>
      ...
      <div class="bg-white rounded-xl border border-slate-200 px-5 py-3.5 flex items-center gap-3 shadow-xs">
        <span class="text-lg font-bold text-slate-700">{{ store.articleCount - store.publishedArticleCount - store.pendingArticleCount }}</span>
        <span class="text-sm text-slate-500 font-medium">Other</span>
      </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-slate-200 shadow-xs overflow-hidden">
      ...
      <div v-if="filtered.length === 0" class="flex flex-col items-center justify-center py-16 gap-3 text-center">
        ...
        <p class="text-sm font-semibold text-slate-600">No articles match your criteria.</p>
        <button class="py-2 px-4 rounded-lg bg-slate-100 hover:bg-slate-200 border border-slate-200 text-xs font-semibold text-slate-600 transition cursor-pointer" @click="searchQuery='';statusFilter='all';categoryFilter='all'">Reset Filters</button>
      </div>
        ...
        <table class="w-full border-collapse text-left">
          <thead>
            <tr class="bg-slate-50 border-b border-slate-200">
              <th class="px-6 py-4 text-[11px] font-bold text-slate-500 uppercase tracking-wider">Article</th>
              ...
              <th class="px-6 py-4 text-[11px] font-bold text-slate-500 uppercase tracking-wider">Views</th>
              <th class="px-6 py-4 text-[11px] font-bold text-slate-500 uppercase tracking-wider">Published</th>
          ...
          <tbody class="divide-y divide-slate-100">
            <tr v-for="article in filtered" :key="article.id" class="hover:bg-slate-50/50 transition-colors">
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <span class="text-sm font-medium text-slate-700">{{ article.author }}</span>
              </td>
              ...
              <td class="px-6 py-4 whitespace-nowrap text-right">
                <div class="flex justify-end gap-1.5">
                  <button class="text-slate-400 hover:text-slate-700 hover:bg-slate-100 p-1.5 rounded-lg transition cursor-pointer" title="View Article" @click="openView(article)">
                    <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                  </button>
                  <button class="text-slate-400 hover:text-slate-700 hover:bg-slate-100 p-1.5 rounded-lg transition cursor-pointer" title="Edit Article" @click="openEdit(article)">
                  ...
                  <button class="text-slate-400 hover:text-red-600 hover:bg-red-50 p-1.5 rounded-lg transition cursor-pointer" title="Delete Article" @click="confirmDel(article)">
