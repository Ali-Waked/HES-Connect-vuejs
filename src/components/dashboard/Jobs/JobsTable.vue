...
const filtered = computed(() => {
  return store.jobs.filter(j => {
    ...
    const matchSearch = !q || 
      localField(j, 'title').toLowerCase().includes(q) || 
  ...
  <div class="space-y-6 animate-fade-in">
    ...
    <div class="flex justify-between items-start">
      <div class="space-y-1">
        ...
        <p class="text-sm text-slate-500">{{ store.jobCount }} {{ $t('jobs.openings') }} {{ $t('jobs.listed') }}</p>
      </div>
      <button
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
      <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 px-5 py-3.5 flex items-center gap-3 shadow-xs">
        <span class="text-lg font-bold text-slate-900 dark:text-white">{{ store.jobCount }}</span>
        <span class="text-sm text-slate-500 font-medium">{{ $t('jobs.total') }}</span>
      </div>
      <div class="bg-white dark:bg-slate-900 rounded-xl border border-emerald-200 dark:border-emerald-800 px-5 py-3.5 flex items-center gap-3 shadow-xs">
        <span class="text-lg font-bold text-emerald-600 dark:text-emerald-400">{{ store.approvedJobCount }}</span>
        <span class="text-sm text-slate-500 font-medium">{{ $t('jobs.approved') }}</span>
      </div>
      <div class="bg-white dark:bg-slate-900 rounded-xl border border-amber-200 dark:border-amber-800 px-5 py-3.5 flex items-center gap-3 shadow-xs">
        <span class="text-lg font-bold text-amber-600 dark:text-amber-400">{{ store.pendingJobCount }}</span>
        <span class="text-sm text-slate-500 font-medium">{{ $t('jobs.pending') }}</span>
      </div>
    </div>
    ...
    <div class="flex flex-col sm:flex-row gap-3">
      <div class="relative flex-grow">
        <svg class="absolute left-3.5 rtl:left-auto rtl:right-3.5 top-1/2 -translate-y-1/2 text-slate-400 w-4 h-4 pointer-events-none" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
          <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
        </svg>
        <input type="text" v-model="searchQuery" class="w-full pl-10 pr-4 rtl:pl-4 rtl:pr-10 py-2.5 text-sm border border-slate-200 dark:border-slate-800 rounded-lg bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none transition" :placeholder="$t('jobs.searchPlaceholder')"/>
      </div>
    ...
    <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-xs overflow-hidden transition-colors">
      ...
      <div v-if="filtered.length === 0" class="flex flex-col items-center justify-center py-16 gap-3 text-center">
        ...
        <button class="py-2 px-4 rounded-lg bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 text-xs font-semibold text-slate-600 dark:text-slate-300 transition cursor-pointer" @click="searchQuery='';statusFilter='all'">{{ $t('common.resetFilters') }}</button>
      </div>
        ...
        <table class="w-full border-collapse text-left rtl:text-right">
          <thead>
            <tr class="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-800">
              ...
              <th class="px-6 py-4 text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">{{ $t('jobs.author') }}</th>
          ...
          <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
            <tr v-for="job in filtered" :key="job.id" class="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors">
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold" :class="statusClass(job.status)">
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex flex-col">
                  ...
                  <span class="text-xs text-slate-600 dark:text-slate-300 font-medium">{{ job.created_date }}</span>
                  <span class="text-[10px] font-bold text-rose-400 uppercase mt-1">{{ $t('jobs.ends') }}</span>
                  <span class="text-xs text-rose-600 dark:text-rose-400 font-bold">{{ job.end_date }}</span>
                </div>
              </td>
              ...
              <td class="px-6 py-4 whitespace-nowrap text-right rtl:text-left">
                <div class="flex justify-end rtl:justify-start gap-1.5">
                  ...
                  <button class="text-slate-400 dark:text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 p-1.5 rounded-lg transition cursor-pointer" :title="$t('common.edit')" @click="openEdit(job)">
    ...
    <ConfirmModal :show="showDeleteModal" :title="$t('jobs.deleteJob')" :message="`${$t('jobs.deleteConfirm', { title: `<strong>${localField(selectedJob, 'title')}</strong>` })}`" :confirm-text="$t('common.delete')" @confirm="handleDelete" @close="showDeleteModal = false"/>
  </div>
</template>
