      ...
      <nav class="flex-grow">
        <ul class="space-y-1.5 list-none">
          <li v-for="item in menuItems" :key="item.id" class="w-full">
            <router-link 
              :to="`/admin/${item.id}`"
              class="flex items-center gap-3 py-3 px-4 text-sm font-medium rounded-lg text-slate-400 hover:bg-white/5 hover:text-white transition duration-200 cursor-pointer" 
            ...
            >
              ...
              <!-- Articles -->
              <svg v-else-if="item.id === 'articles'" class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" />
              </svg>
              <!-- Jobs -->
              <svg v-else-if="item.id === 'jobs'" class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
