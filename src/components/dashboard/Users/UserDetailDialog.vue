    ...
    <div v-if="user" class="space-y-8">
      ...
      <div class="relative">
        ...
        <div class="absolute -bottom-6 left-6 flex items-end gap-4">
          ...
          <div class="mb-2 pb-1">
            <h4 class="text-xl font-black text-slate-900 leading-none">{{ user.name }}</h4>
            <p class="text-xs font-bold text-slate-400 mt-1 uppercase tracking-widest">{{ type }} • Joined {{ user.joined || 'Recently' }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 mt-12">
        ...
        <div class="lg:col-span-2 space-y-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="space-y-1">
              ...
              <p class="text-sm font-bold text-slate-700">{{ user.profile?.phone || 'Not provided' }}</p>
            </div>
            <div class="space-y-1">
              <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Gender</label>
              <p class="text-sm font-bold text-slate-700">{{ user.profile?.gender || 'Not specified' }}</p>
            </div>
            <div class="space-y-1">
              <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Birth Date</label>
              <p class="text-sm font-bold text-slate-700">{{ user.profile?.birthDate || 'Not provided' }}</p>
            </div>
            <div class="md:col-span-2 space-y-1">
              <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Home Address</label>
              <p class="text-sm font-bold text-slate-700">{{ user.profile?.address || 'No address on file' }}</p>
            </div>
          </div>

          <!-- Permissions Section -->
          <div class="pt-6 border-t border-slate-100 space-y-4">
            <label class="block text-xs font-bold text-slate-900 uppercase tracking-wider">Inherited Permissions</label>
              ...
              <div v-for="(perms, group) in groupedPermissions" :key="group" class="space-y-2">
                ...
                <div class="flex flex-wrap gap-1.5">
                  <span 
                    v-for="perm in perms" 
                    :key="perm.id"
                    class="px-2 py-0.5 bg-slate-100 text-slate-600 text-[10px] font-bold rounded uppercase tracking-tighter"
                  >
        ...
        <div class="space-y-6">
          <div class="p-6 bg-slate-50 rounded-2xl border border-slate-100 space-y-6">
            <div class="space-y-3">
              <label class="block text-xs font-bold text-slate-900 uppercase tracking-wider">Access Role</label>
              <select 
                :value="user.role_id"
                @change="handleRoleChange"
                class="w-full p-3 text-sm font-bold border border-slate-200 rounded-xl focus:outline-none transition cursor-pointer bg-white shadow-sm"
              >
            ...
            <div class="space-y-3 pt-6 border-t border-slate-200">
              <button class="w-full py-3 bg-white border border-slate-200 text-slate-700 text-xs font-black uppercase tracking-widest rounded-xl hover:bg-slate-100 transition">Reset Password</button>
              <button class="w-full py-3 bg-white border border-slate-200 text-rose-600 text-xs font-black uppercase tracking-widest rounded-xl hover:bg-rose-50 hover:border-rose-100 transition">Suspend User</button>
            </div>
          </div>

          <div class="p-4 flex items-center justify-between">
            <span class="text-xs text-slate-400 font-medium italic">Last updated: Today, 10:20 AM</span>
          </div>
        </div>
      </div>
