...
const confirmDel = (member) => { selectedMember.value = member; showDeleteModal.value = true; };
const handleDelete = () => {
    ...
    <div class="flex justify-between items-start">
      <div class="flex items-start gap-3">
        ...
        <div>
          ...
          <p class="text-sm text-slate-500 mt-1">{{ filtered.length }} registered staff {{ filtered.length === 1 ? 'member' : 'members' }}</p>
        </div>
      </div>
            ...
            <tr v-for="member in filtered" :key="member.id" class="hover:bg-slate-50/50 transition-colors">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-right">
                <div class="flex justify-end gap-1.5">
                  ...
                  <button class="text-slate-400 hover:text-red-600 hover:bg-red-50 p-1.5 rounded-lg transition cursor-pointer" title="Delete" @click="confirmDel(member)">
                    <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
