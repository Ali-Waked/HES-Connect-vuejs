...
const { categories, loading, stats, statsLoading, deleteCategory, pagination, fetchCategories, fetchCategoryStats } = useCategories();
...
function typeLabel(type) {
  const map = {
    article: t('categoryManagement.article'),
    story: t('categoryManagement.story'),
    job: t('categoryManagement.job'),
  }
...
watch(showFormModal, (val) => {
  if (!val) {
    fetchCategories(parseParams(route.query))
    fetchCategoryStats()
  }
})
...
const handleDelete = async () => {
  if (selectedCategory.value) {
    await deleteCategory(selectedCategory.value.uuid);
...
<template>
  <div class="space-y-6 animate-fade-in">
    ...
    <div class="flex justify-between items-start">
      <div class="space-y-1">
        <h1 class="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">{{ $t('categoryManagement.title') }}</h1>
        <p class="text-sm text-slate-500 dark:text-slate-400">{{ pagination.total || 0 }} {{ $t('categoryManagement.registered') || 'categories' }}</p>
      </div>
      <button
    ...
    <div v-if="stats" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
      <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-5 flex items-center gap-4">
        ...
        <div>
          <p class="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">{{ $t('categoryManagement.total') || 'Total' }}</p>
          <p class="text-2xl font-black text-slate-900 dark:text-white">{{ stats.total }}</p>
          <p class="text-[11px] text-slate-400 dark:text-slate-500 mt-0.5">{{ $t('categoryManagement.totalDesc') || 'All categories in the system' }}</p>
        </div>
      </div>
      <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-5 flex items-center gap-4">
        ...
        <div>
          <p class="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">{{ $t('categoryManagement.articleCategories') || 'Articles' }}</p>
          <p class="text-2xl font-black text-slate-900 dark:text-white">{{ stats.articles }}</p>
          <p class="text-[11px] text-slate-400 dark:text-slate-500 mt-0.5">{{ $t('categoryManagement.articleDesc') || 'Categories used for articles' }}</p>
        </div>
      </div>
      <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-5 flex items-center gap-4">
        ...
        <div>
          <p class="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">{{ $t('categoryManagement.storyCategories') || 'Stories' }}</p>
          <p class="text-2xl font-black text-slate-900 dark:text-white">{{ stats.stories }}</p>
          <p class="text-[11px] text-slate-400 dark:text-slate-500 mt-0.5">{{ $t('categoryManagement.storyDesc') || 'Categories used for stories' }}</p>
        </div>
      </div>
      <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-5 flex items-center gap-4">
        ...
        <div>
          <p class="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">{{ $t('categoryManagement.jobCategories') || 'Jobs' }}</p>
          <p class="text-2xl font-black text-slate-900 dark:text-white">{{ stats.jobs }}</p>
          <p class="text-[11px] text-slate-400 dark:text-slate-500 mt-0.5">{{ $t('categoryManagement.jobDesc') || 'Categories used for jobs' }}</p>
        </div>
      </div>
      <!-- <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-5 flex items-center gap-4">
        ...
        <div>
          <p class="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">{{ $t('categoryManagement.active') || 'Active' }}</p>
          <p class="text-2xl font-black text-slate-900 dark:text-white">{{ stats.active }}</p>
          <p class="text-[11px] text-slate-400 dark:text-slate-500 mt-0.5">{{ $t('categoryManagement.activeDesc') || 'Currently active categories' }}</p>
        </div>
      </div>
      <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-5 flex items-center gap-4">
        ...
        <div>
          <p class="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">{{ $t('categoryManagement.inactive') || 'Inactive' }}</p>
          <p class="text-2xl font-black text-slate-900 dark:text-white">{{ stats.inactive }}</p>
          <p class="text-[11px] text-slate-400 dark:text-slate-500 mt-0.5">{{ $t('categoryManagement.inactiveDesc') || 'Disabled categories' }}</p>
        </div>
      </div> -->
    </div>

    <!-- Stats Loading Skeleton -->
    <div v-else-if="statsLoading" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
      <div v-for="i in 6" :key="i" class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-5 flex items-center gap-4 animate-pulse">
    ...
    <div class="flex flex-col sm:flex-row gap-3 flex-wrap">
      <div class="relative flex-grow min-w-[200px]">
        ...
        <input
          ...
          :placeholder="$t('categoryManagement.searchPlaceholder') || 'Search categories...'"
          :value="searchQuery"
      ...
      >
        <option value="all">{{ $t('categoryManagement.filterType') || 'All Types' }}</option>
    ...
    <div v-else class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-xs overflow-hidden">
      <!-- Empty state -->
      <div v-if="categories?.length === 0" class="flex flex-col items-center justify-center py-12 px-6 text-center gap-4">
        ...
        <p class="text-sm font-semibold text-slate-600 dark:text-slate-400">{{ $t('categoryManagement.noResults') || 'No categories found.' }}</p>
        ...
        <table class="w-full border-collapse text-left rtl:text-right">
          <thead>
            <tr class="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-800">
              <th class="px-6 py-4 text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider w-[28%]">{{ $t('categoryManagement.name') || 'Name' }}</th>
              <th class="px-6 py-4 text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider w-[12%]">{{ $t('categoryManagement.type') || 'Type' }}</th>
              <th class="px-6 py-4 text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider w-[12%]">{{ $t('common.status') }}</th>
          ...
          <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
            <tr v-for="category in categories" :key="category.uuid || category.id" class="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex flex-col gap-0.5">
                  <span class="text-sm font-semibold text-slate-900 dark:text-white">{{ localField(category, 'name') }}</span>
                  <span v-if="localField(category, 'description')" class="text-xs text-slate-400 dark:text-slate-500 truncate max-w-[300px]">{{ localField(category, 'description').split('\n')[0] }}</span>
                </div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold" :class="{
                  'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400': category.type === 'article',
                  'bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-400': category.type === 'story',
                  'bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-400': category.type === 'job',
                }">
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold" :class="{
                  'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-400': category.is_active === 1 || category.is_active === true,
                  'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400': category.is_active === 0 || category.is_active === false,
                }">
                  {{ (category.is_active === 1 || category.is_active === true) ? $t('statuses.active') : $t('statuses.inactive') }}
                </span>
              </td>
              ...
              <td class="px-6 py-4 whitespace-nowrap text-right rtl:text-left">
                <div class="flex justify-end rtl:justify-start gap-1.5">
                  <button class="text-slate-400 dark:text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 p-1.5 rounded-lg transition cursor-pointer" :title="$t('common.edit')" @click="openEditModal(category)">
                    <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                      <path stroke-linecap="round" stroke-linejoin="round" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                    </svg>
                  </button>
                  <button class="text-slate-400 dark:text-slate-500 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 p-1.5 rounded-lg transition cursor-pointer" :title="$t('common.delete')" @click="confirmDelete(category)">
                    <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    <CategoryFormModal :show="showFormModal" :category="selectedCategory" @close="showFormModal = false" />
    <ConfirmModal
      :show="showDeleteModal"
      :title="$t('categoryManagement.deleteCategory') || 'Delete Category'"
      :message="$t('categoryManagement.deleteConfirm', { name: localField(selectedCategory, 'name') })"
