  ...
  <div class="min-h-screen bg-landing-cream font-sans antialiased text-landing-dark">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-10 pt-28 lg:pb-14 lg:pt-32">
      <div data-aos="fade-up">
        ...
        <p class="text-slate-500 mb-8">{{ filtered.length }} facilities found</p>
      </div>
      ...
      <div v-else-if="filtered.length === 0" class="text-center py-16 text-slate-500">
      ...
      <div v-else class="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
        <div
          ...
          class="bg-white rounded-xl border border-slate-200 p-5 shadow-sm hover:shadow-md transition"
        >
          ...
          <h3 class="text-base font-bold text-slate-900 mb-1">{{ fac.name }}</h3>
          <p class="text-sm text-slate-500 mb-3">{{ fac.organization_name }}</p>
          <div class="flex items-center gap-1 text-sm text-amber-500">
            <span class="font-bold text-slate-700">{{ fac.rating }}</span>
