...
const typeBadge = {
  hospital: 'bg-red-100 text-red-700/30',
  clinic: 'bg-blue-100 text-blue-700/30',
  pharmacy: 'bg-emerald-100 text-emerald-700/30',
  medical_point: 'bg-orange-100 text-orange-700/30'
};
</script>
    ...
    <main class="pt-20 lg:pt-[90px]">
      <template v-if="loading">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
          ...
          <div class="aspect-video bg-slate-200 rounded-xl animate-pulse"></div>
      ...
      <template v-else-if="notFound">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 text-center">
          ...
          <h2 class="text-xl font-bold text-slate-900 mb-2">Facility Not Found</h2>
          <p class="text-sm text-slate-500 mb-6">The facility you're looking for doesn't exist or has been removed.</p>
            ...
            <div class="flex items-center gap-4">
              ...
              <div>
                <div class="flex items-center gap-2 mb-1">
                  <span class="px-2.5 py-0.5 rounded-full text-xs font-semibold border capitalize" :class="typeBadge[facility.facility_type] || 'bg-slate-100 text-slate-600'">
                  ...
                  <div class="flex items-center gap-1 text-sm">
                    ...
                    <span class="font-semibold text-slate-700">{{ facility.rating }}</span>
                    <span class="text-slate-400">({{ facility.review_count }} reviews)</span>
                  </div>
                </div>
                <h1 class="text-2xl lg:text-3xl font-bold text-slate-900">{{ facility.name }}</h1>
                <p class="text-sm text-slate-500 mt-1">{{ facility.organization_name }}</p>
              </div>
            </div>
          ...
          <div class="grid lg:grid-cols-3 gap-8">
            <div class="lg:col-span-2 space-y-10">
              ...
              <div data-aos="fade-up">
                <h2 class="text-lg font-bold text-slate-900 mb-3">About this Facility</h2>
              ...
              <div v-if="facility.doctors && facility.doctors.length > 0" data-aos="fade-up">
                <h2 class="text-lg font-bold text-slate-900 mb-4">Our Doctors</h2>
                <Swiper :modules="[Pagination]" :pagination="{ clickable: true, dynamicBullets: true }" :space-between="16" :slides-per-view="'auto'" class="pb-8">
                  <SwiperSlide v-for="doc in facility.doctors" :key="doc.id" class="!w-[220px]">
                    <div class="bg-white rounded-xl border border-landing-border p-5 shadow-sm text-center hover:shadow-md hover:-translate-y-0.5 transition-all duration-200">
                      ...
                      <h3 class="text-sm font-bold text-slate-900 mb-1">{{ doc.name }}</h3>
                      <p class="text-xs text-slate-500 mb-2">{{ doc.specialization }}</p>
            ...
            <!-- Sidebar -->
            <div class="space-y-6">
              <div data-aos="fade-up" class="bg-white rounded-xl border border-landing-border p-5 shadow-sm">
                <h3 class="text-sm font-bold text-slate-900 mb-4">Facility Information</h3>
                <div class="space-y-3">
                  <div class="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p class="text-sm font-semibold text-slate-700">{{ facility.location }}</p>
                    </div>
                  </div>
                  <div v-if="facility.phone" class="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p class="text-sm font-semibold text-slate-700">{{ facility.phone }}</p>
                    </div>
                  </div>
                </div>
              </div>
