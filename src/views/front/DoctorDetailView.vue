    ...
    <main class="pt-20 lg:pt-[90px]">
      <template v-if="loading">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
          ...
          <div class="flex flex-col lg:flex-row gap-8">
            <div class="w-full lg:w-1/3 aspect-square bg-slate-200 dark:bg-slate-700 rounded-2xl animate-pulse"></div>
      ...
      <template v-else-if="notFound">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 text-center">
          ...
          <h2 class="text-xl font-bold text-slate-900 dark:text-slate-100 mb-2">Doctor Not Found</h2>
          <p class="text-sm text-slate-500 dark:text-slate-400 mb-6">The doctor you're looking for doesn't exist or has been removed.</p>
      ...
      <template v-else-if="doctor">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 pb-2">
          <nav class="flex items-center gap-2 text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">
            ...
            <span class="text-slate-600 dark:text-slate-400">Doctors</span>
            <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" d="M8.25 4.5l7.5 7.5-7.5 7.5"/></svg>
            <span class="text-slate-600 dark:text-slate-400">{{ doctor.name }}</span>
          </nav>
        </div>

        <!-- Profile Header -->
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div class="bg-white dark:bg-slate-800 rounded-2xl border border-landing-border dark:border-slate-700 shadow-sm overflow-hidden">
              ...
              <div class="flex flex-col lg:flex-row lg:items-end gap-6">
                ...
                <div class="flex-1 min-w-0 pt-4 lg:pt-0">
                  <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                    <div>
                      <h1 class="text-xl lg:text-2xl font-bold text-slate-900 dark:text-slate-100">{{ doctor.name }}</h1>
                      <p class="text-sm text-brand-primary font-semibold mt-0.5">{{ doctor.specialization }}</p>
                      <div class="flex flex-wrap items-center gap-3 mt-2 text-sm text-slate-500 dark:text-slate-400">
                    ...
                    <div class="flex items-center gap-4">
                      <div class="text-center">
                        <div class="text-2xl font-extrabold text-slate-900 dark:text-slate-100">{{ doctor.rating }}</div>
                        <div class="flex gap-0.5 mt-0.5">
                          <svg v-for="s in 5" :key="s" class="w-3.5 h-3.5" :class="s <= Math.round(doctor.rating) ? 'text-amber-400' : 'text-slate-200 dark:text-slate-500'" fill="currentColor" viewBox="0 0 20 20">
                            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                          </svg>
                        </div>
                        <p class="text-xs text-slate-400 dark:text-slate-500 mt-0.5">{{ stats.total }} reviews</p>
                      </div>
                      <div class="text-center px-4 border-l border-landing-border dark:border-slate-700">
                        <div class="text-2xl font-extrabold text-brand-primary">${{ doctor.fee }}</div>
                        <p class="text-xs text-slate-400 dark:text-slate-500 mt-0.5">Consultation fee</p>
                      </div>
                    </div>
                  </div>
                </div>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
          <div class="grid lg:grid-cols-3 gap-8">
            <div class="lg:col-span-2 space-y-10">
              <!-- Gallery -->
              <div v-if="hasGallery">
                <h2 class="text-lg font-bold text-slate-900 dark:text-slate-100 mb-4">Gallery</h2>
                <MediaGallery :images="doctor.gallery" aspect-ratio="aspect-video" />
              </div>

              <!-- About -->
              <div>
                <h2 class="text-lg font-bold text-slate-900 dark:text-slate-100 mb-3">About</h2>
                <p class="text-sm text-slate-600 dark:text-slate-400 leading-relaxed">{{ doctor.about }}</p>
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-4 mt-6">
                  <div class="bg-white dark:bg-slate-800 rounded-xl border border-landing-border dark:border-slate-700 p-4 text-center">
                    <p class="text-lg font-extrabold text-slate-900 dark:text-slate-100">{{ doctor.experience }}+</p>
                    <p class="text-xs text-slate-500 dark:text-slate-400">Years Experience</p>
                  </div>
                  <div class="bg-white dark:bg-slate-800 rounded-xl border border-landing-border dark:border-slate-700 p-4 text-center">
                    <p class="text-lg font-extrabold text-slate-900 dark:text-slate-100">{{ stats.total }}+</p>
                    <p class="text-xs text-slate-500 dark:text-slate-400">Patient Reviews</p>
                  </div>
                  <div class="bg-white dark:bg-slate-800 rounded-xl border border-landing-border dark:border-slate-700 p-4 text-center">
                    <p class="text-lg font-extrabold text-brand-primary">${{ doctor.fee }}</p>
                    <p class="text-xs text-slate-500 dark:text-slate-400">Consultation Fee</p>
                  </div>
                </div>
              </div>
              ...
              <div v-if="doctor.related_doctors && doctor.related_doctors.length > 0">
                <h2 class="text-lg font-bold text-slate-900 dark:text-slate-100 mb-4">Related Doctors</h2>
                <Swiper :modules="[Pagination]" :pagination="{ clickable: true, dynamicBullets: true }" :space-between="16" :slides-per-view="'auto'" class="pb-8">
                  <SwiperSlide v-for="doc in doctor.related_doctors" :key="doc.id" class="!w-[200px]">
                    <div class="bg-white dark:bg-slate-800 rounded-xl border border-landing-border dark:border-slate-700 p-4 shadow-sm text-center hover:shadow-md hover:-translate-y-0.5 transition-all duration-200">
                      ...
                      <h3 class="text-sm font-bold text-slate-900 dark:text-slate-100 mb-1 line-clamp-1">{{ doc.name }}</h3>
                      <p class="text-xs text-slate-500 dark:text-slate-400 mb-2 line-clamp-1">{{ doc.specialization }}</p>
                      <div class="flex items-center justify-center gap-1 mb-3">
                        <svg v-for="s in 5" :key="s" class="w-3 h-3" :class="s <= Math.round(doc.rating) ? 'text-amber-400' : 'text-slate-200 dark:text-slate-500'" fill="currentColor" viewBox="0 0 20 20">
                          <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                        </svg>
                      </div>
            ...
            <!-- Sidebar -->
            <div class="space-y-6">
              <div class="bg-white dark:bg-slate-800 rounded-xl border border-landing-border dark:border-slate-700 p-5 shadow-sm">
                <h3 class="text-sm font-bold text-slate-900 dark:text-slate-100 mb-4">Booking Info</h3>
                <div class="space-y-3">
                  <div class="flex items-start gap-3">
                    <svg class="w-4 h-4 text-slate-400 dark:text-slate-500 mt-0.5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" d="M15 10.5a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z"/></svg>
                    <div>
                      <p class="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Location</p>
                      <p class="text-sm font-semibold text-slate-700 dark:text-slate-300">{{ doctor.facility?.name }}</p>
                    </div>
                  </div>
                  <div class="flex items-start gap-3">
                    <svg class="w-4 h-4 text-slate-400 dark:text-slate-500 mt-0.5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                    <div>
                      <p class="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Experience</p>
                      <p class="text-sm font-semibold text-slate-700 dark:text-slate-300">{{ doctor.experience }} years</p>
                    </div>
                  </div>
                  <div class="flex items-start gap-3">
                    ...
                    <div>
                      <p class="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Fee</p>
