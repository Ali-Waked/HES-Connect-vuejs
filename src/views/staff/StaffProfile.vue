<template>
  <div class="animate-fade-in space-y-6">
    <div>
      ...
      <p class="text-sm text-slate-500">Manage your personal information and security</p>
    </div>
        ...
        <section class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div class="space-y-4">
            <div>
              ...
              <input v-model="form.name" class="w-full rounded-lg border border-slate-200 px-3 py-2.5 text-sm" />
            </div>
            <div>
              <label class="mb-1 block text-xs font-semibold text-slate-500">Email</label>
              <input :value="store.currentUser.email" class="w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm" readonly />
            </div>
            <div class="grid grid-cols-2 gap-4">
              <div>
                <label class="mb-1 block text-xs font-semibold text-slate-500">Phone</label>
                <input v-model="form.phone" class="w-full rounded-lg border border-slate-200 px-3 py-2.5 text-sm" />
              </div>
              <div>
                <label class="mb-1 block text-xs font-semibold text-slate-500">Gender</label>
                <select v-model="form.gender" class="w-full rounded-lg border border-slate-200 px-3 py-2.5 text-sm">
            ...
            <div class="grid grid-cols-2 gap-4">
              <div>
                <label class="mb-1 block text-xs font-semibold text-slate-500">Birth Date</label>
                <input type="date" v-model="form.birthDate" class="w-full rounded-lg border border-slate-200 px-3 py-2.5 text-sm" />
              </div>
              <div>
                <label class="mb-1 block text-xs font-semibold text-slate-500">Address</label>
        ...
        <section class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div class="space-y-4">
            <div>
              ...
              <input type="password" v-model="password.current" class="w-full rounded-lg border border-slate-200 px-3 py-2.5 text-sm" />
            </div>
            <div>
              <label class="mb-1 block text-xs font-semibold text-slate-500">New Password</label>
              <input type="password" v-model="password.newPass" class="w-full rounded-lg border border-slate-200 px-3 py-2.5 text-sm" />
            </div>
            <div>
              <label class="mb-1 block text-xs font-semibold text-slate-500">Confirm Password</label>
              <input type="password" v-model="password.confirm" class="w-full rounded-lg border border-slate-200 px-3 py-2.5 text-sm" />
            </div>
            <p v-if="passwordError" class="text-xs text-red-500">{{ passwordError }}</p>
            <button class="rounded-md border border-slate-200 px-6 py-2.5 text-sm font-semibold text-slate-700" @click="updatePassword">Update Password</button>
          </div>

          <div class="mt-8 border-t border-slate-100 pt-6">
            <h4 class="font-bold text-slate-900 mb-4">Two-Factor Authentication</h4>
            <div class="flex items-center justify-between">
              <span class="text-sm text-slate-700">Enable 2FA</span>
              <button class="h-6 w-11 rounded-full transition" :class="twoFactor ? 'bg-brand-primary' : 'bg-slate-200'" @click="twoFactor = !twoFactor">
            ...
            <div v-if="twoFactor" class="mt-4 rounded-lg border border-slate-100 p-4 text-center">
              <div class="mx-auto h-24 w-24 rounded-lg bg-slate-100 flex items-center justify-center text-slate-400 text-xs">QR Code Placeholder</div>
              <p class="mt-2 text-xs text-slate-500">Scan with your authenticator app</p>
            </div>
          </div>
        </section>
      </div>
