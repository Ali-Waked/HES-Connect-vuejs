<template>
  <div class="animate-fade-in space-y-6">
    <div class="flex items-center justify-between">
      <div>
        <h2 class="text-xl font-bold text-slate-900 dark:text-slate-100">My Schedule</h2>
        <p class="text-sm text-slate-500 dark:text-slate-400">Weekly slots and blocked dates</p>
      </div>
      <div class="flex gap-2">
        <button class="rounded-md border border-slate-200 dark:border-slate-700 px-4 py-2 text-sm text-slate-700 dark:text-slate-300" @click="blockModal = true">Block Date</button>
    ...
    <template v-else>
      <section class="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-6 shadow-sm">
        <h3 class="text-lg font-bold text-slate-900 dark:text-slate-100 mb-4">Weekly View</h3>
        <div class="grid grid-cols-7 gap-2">
          <div v-for="(day, i) in dayNames" :key="i" class="rounded-lg border border-slate-100 dark:border-slate-700 p-3 min-h-[120px]">
            <div class="text-center border-b border-slate-100 dark:border-slate-700 pb-2 mb-2">
              <p class="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase">{{ day }}</p>
            </div>
            <div v-for="slot in getSlotsForDay(i)" :key="slot.id" class="mb-2 rounded-md bg-brand-primary/10 p-2 text-xs">
              ...
              <span class="text-slate-500 dark:text-slate-400">{{ slot.slotDuration }}min</span>
            </div>
          </div>
        </div>
      </section>
      ...
      <section class="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-6 shadow-sm">
        <div class="flex items-center justify-between mb-4">
          <div>
            <h3 class="text-lg font-bold text-slate-900 dark:text-slate-100">Unavailable Dates</h3>
            <p class="text-sm text-slate-500 dark:text-slate-400">Blocked days</p>
          </div>
        </div>
        <div v-if="store.unavailableDates.length" class="space-y-2">
          <div v-for="d in store.unavailableDates" :key="d.id" class="flex items-center justify-between rounded-lg border border-slate-100 dark:border-slate-700 p-3">
            <div>
              <span class="font-semibold text-slate-900 dark:text-slate-100">{{ d.date }}</span>
              <span class="ml-2 rounded-full bg-slate-100 dark:bg-slate-700 px-2.5 py-0.5 text-xs text-slate-600 dark:text-slate-300">{{ d.reason }}</span>
            </div>
            <button class="text-red-400 hover:text-red-600 text-sm" @click="confirmDeleteDate(d)">✕</button>
          </div>
        </div>
        <div v-else class="py-6 text-center text-sm text-slate-500 dark:text-slate-400">No unavailable dates.</div>
      </section>
    </template>
