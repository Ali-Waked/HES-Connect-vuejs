<template>
  <div class="animate-fade-in space-y-6">
    ...
    <template v-else>
      <section class="rounded-xl border border-red-200 dark:border-red-900 bg-red-50/40 dark:bg-red-900/20 p-6 shadow-sm">
        <div class="flex items-center gap-2">
          <span class="material-symbols-outlined text-red-600">warning</span>
          <h3 class="text-lg font-bold text-slate-900 dark:text-slate-100">Low Stock Alert</h3>
        </div>
        <p class="text-sm text-slate-500 dark:text-slate-400">Items with quantity below 10</p>
        <div class="mt-4 space-y-3">
          <div v-if="store.lowStockItems.length" v-for="item in store.lowStockItems" :key="item.id" class="flex items-center justify-between rounded-lg bg-white dark:bg-slate-800 border border-red-100 dark:border-red-900 p-3">
            <div>
              <p class="font-semibold text-slate-900 dark:text-slate-100">{{ item.medicineName }}</p>
              <p class="text-xs text-slate-500 dark:text-slate-400">Quantity: {{ item.quantity }}</p>
            </div>
          ...
          <div v-else class="py-4 text-sm text-slate-500 dark:text-slate-400">All items are well-stocked.</div>
        </div>
      </section>

      <section class="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-6 shadow-sm">
        <div>
          <h3 class="text-lg font-bold text-slate-900 dark:text-slate-100">Recent Medication Requests</h3>
          <p class="text-sm text-slate-500 dark:text-slate-400">Last 5 requests</p>
        </div>
        <div class="mt-4">
            ...
            <table class="w-full text-left">
              <thead class="border-b border-slate-200 dark:border-slate-700">
                <tr>
                  ...
                  <th class="px-4 py-3 text-xs font-bold uppercase text-slate-500 dark:text-slate-400">Medicines</th>
                  <th class="px-4 py-3 text-xs font-bold uppercase text-slate-500 dark:text-slate-400">Status</th>
                  <th class="px-4 py-3 text-xs font-bold uppercase text-slate-500 dark:text-slate-400">Date</th>
                  <th class="px-4 py-3 text-xs font-bold uppercase text-slate-500 dark:text-slate-400"></th>
                </tr>
              </thead>
              <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                <tr v-for="req in recentRequests" :key="req.id" class="transition hover:bg-slate-50/70 dark:hover:bg-slate-700/70">
                  <td class="px-4 py-3 font-semibold text-slate-900 dark:text-slate-100">{{ req.patientName }}</td>
                  <td class="px-4 py-3 text-sm text-slate-500 dark:text-slate-400">{{ req.medicines.length }}</td>
                  <td class="px-4 py-3"><StatusBadge :status="req.status" /></td>
                  <td class="px-4 py-3 text-sm text-slate-500 dark:text-slate-400">{{ req.date }}</td>
                  <td class="px-4 py-3">
                    <div class="flex gap-2" v-if="req.status === 'pending'">
                      ...
                      <button class="rounded-md border border-slate-200 dark:border-slate-700 px-3 py-1.5 text-sm text-slate-700 dark:text-slate-300" @click="reject(req)">Reject</button>
                    </div>
                  </td>
                </tr>
              </tbody>
          ...
          <div v-else class="py-8 text-center">
            <span class="material-symbols-outlined text-4xl text-slate-300 dark:text-slate-500">inbox</span>
            <p class="mt-2 text-sm font-bold text-slate-900 dark:text-slate-100">No recent requests</p>
          </div>
        </div>
      </section>
    </template>
