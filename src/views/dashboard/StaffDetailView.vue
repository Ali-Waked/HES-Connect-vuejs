...
<template>
  <div v-if="member" class="space-y-8 animate-fade-in">
    <!-- Profile Header -->
    <div class="bg-white p-8 rounded-2xl border border-slate-100 shadow-sm flex flex-col md:flex-row gap-8 items-center md:items-start">
      ...
      <div class="flex-grow text-center md:text-left space-y-4">
        <div class="space-y-1">
          <nav class="flex items-center justify-center md:justify-start gap-2 text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">
            <button @click="goBack" class="hover:text-brand-primary transition">Staff</button>
            <span class="material-symbols-outlined text-[10px]">chevron_right</span>
            <span class="text-slate-600">Profile Detail</span>
          </nav>
          <h1 class="text-3xl font-black text-slate-900 tracking-tight">{{ member.name }}</h1>
          <div class="flex flex-wrap justify-center md:justify-start items-center gap-3 mt-2">
            ...
            <span class="inline-flex items-center gap-1.5 text-sm font-semibold" :class="member.status === 'Online' ? 'text-emerald-600' : 'text-slate-400'">
        ...
        <div class="flex flex-wrap justify-center md:justify-start gap-6 py-4 border-t border-slate-50 mt-4">
          <div class="space-y-0.5">
            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest text-center md:text-left">Email Address</p>
            <p class="text-sm font-semibold text-slate-700">{{ member.email }}</p>
          </div>
          <div class="space-y-0.5">
            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest text-center md:text-left">Consultation Fee</p>
            <p class="text-sm font-bold text-brand-primary">${{ member.fee }}/visit</p>
          </div>
          <div class="space-y-0.5">
            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest text-center md:text-left">Auth Provider</p>
            <p class="text-sm font-semibold text-slate-700">{{ member.provider }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="flex flex-col gap-2 shrink-0 w-full md:w-auto">
        ...
        <button class="w-full px-6 py-3 bg-white border border-slate-200 text-slate-700 text-sm font-bold rounded-xl hover:bg-slate-50 transition">
          Message Doctor
        </button>
      </div>
    </div>
    ...
    <div class="space-y-6">
      ...
      <div class="pt-4 border-t border-slate-100">
        ...
        >
          ...
          <div v-if="!reviewStore.hasMore" class="py-12 text-center bg-slate-50 rounded-2xl border border-dashed border-slate-200 mt-4">
            <span class="material-symbols-outlined text-slate-300 text-4xl mb-2">done_all</span>
            <p class="text-sm font-bold text-slate-400 uppercase tracking-widest">End of results</p>
          </div>
        </InfiniteScroll>
      </div>
    </div>
