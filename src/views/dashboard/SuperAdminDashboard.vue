...
<template>
  <div class="space-y-8 animate-fade-in">
    ...
    <div class="flex justify-between items-end">
      <div>
        <h1 class="text-3xl font-black text-slate-900 tracking-tight">Super Admin Console</h1>
        <p class="text-sm text-slate-500 mt-1 font-medium">Global platform oversight and operational control</p>
      </div>
      <div class="flex gap-3">
        <button class="px-4 py-2 bg-white border border-slate-200 text-slate-600 text-sm font-bold rounded-xl hover:bg-slate-50 transition flex items-center gap-2 shadow-sm">
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-8">
        ...
        <div class="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
          ...
          <div class="space-y-6">
            <div v-for="(act, idx) in recentActivity" :key="idx" class="flex gap-4 relative">
              <div v-if="idx !== recentActivity.length - 1" class="absolute left-5 top-10 bottom-[-24px] w-px bg-slate-100"></div>
              ...
              <div class="min-w-0 flex-grow">
                <div class="flex justify-between items-start">
                  <h4 class="text-sm font-bold text-slate-900 truncate">{{ act.user }}</h4>
                  <span class="text-[10px] font-bold text-slate-400 uppercase whitespace-nowrap">{{ act.time }}</span>
                </div>
                <p class="text-xs text-slate-500 mt-1 leading-relaxed">{{ act.detail || 'New user joined the platform' }}</p>
              </div>
            </div>
          </div>

          <button class="w-full mt-8 py-3 text-xs font-black text-slate-400 hover:text-brand-primary transition uppercase tracking-widest border-t border-slate-50">
            View Full Audit Log
          </button>
        </div>
        ...
        <div class="bg-slate-900 p-6 rounded-2xl shadow-xl shadow-slate-900/10">
          ...
          <div class="space-y-4">
            <div v-for="sys in ['API Server', 'Database', 'File Storage', 'Auth Service']" :key="sys" class="flex items-center justify-between">
              <span class="text-sm text-slate-400 font-medium">{{ sys }}</span>
      ...
      <div class="xl:col-span-2 space-y-8">
        ...
        <div class="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm h-[400px] flex flex-col">
          <div class="flex justify-between items-center mb-8">
            <h3 class="text-lg font-bold text-slate-900">Operational Trends</h3>
            <div class="flex gap-2">
              <button class="px-3 py-1 bg-slate-100 text-slate-600 text-[10px] font-black rounded-md">30D</button>
              <button class="px-3 py-1 text-slate-400 text-[10px] font-black rounded-md">90D</button>
            </div>
          </div>
          <div class="flex-grow flex items-center justify-center bg-slate-50 rounded-xl border border-dashed border-slate-200">
            <div class="text-center">
              <span class="material-symbols-outlined text-4xl text-slate-300 mb-2">monitoring</span>
              <p class="text-sm font-bold text-slate-400 uppercase tracking-widest">Chart Visualization Container</p>
              <p class="text-xs text-slate-400 mt-1">Appointments & User Growth over time</p>
            </div>
          </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
          <div class="px-6 py-4 border-b border-slate-50 flex justify-between items-center">
            <h3 class="text-lg font-bold text-slate-900">High-Priority Medication Requests</h3>
          ...
          <table class="w-full text-left">
            ...
            <tbody class="divide-y divide-slate-50">
              <tr v-for="req in clinicalStore.medicationRequests.filter(r => r.status === 'Pending')" :key="req.id" class="hover:bg-slate-50/30 transition-colors">
                <td class="px-6 py-4">
                  <span class="text-sm font-bold text-slate-900">{{ req.patient }}</span>
                </td>
                <td class="px-6 py-4">
                  <span class="text-sm text-slate-600">{{ req.facility }}</span>
                </td>
                <td class="px-6 py-4">
